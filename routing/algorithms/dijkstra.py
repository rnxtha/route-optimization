...
class Dijkstra(RouteAlgorithm):
    ...
    @staticmethod
    def _get_edges_on_path(graph, path: List[int]) -> List[Any]:
        edges = []
    ...
    def find_route(self, graph, start_node_id: int, end_node_id: int,
        ...
        # Cache: edge -> (weight, detail) to avoid re-running the ML predictor
        ...
        # Sum real distance (meters) and aggregate ML travel info
        total_distance = 0.0
        total_travel_seconds = 0.0
        edges_detail: List[Dict[str, Any]] = []
        for i in range(len(path) - 1):
            u, v = path[i], path[i + 1]
            weight, distance_m, detail = weight_cache.get((u, v), (0.0, 0.0, {}))
        ...
        # Aggregate traffic stats from the path edges
